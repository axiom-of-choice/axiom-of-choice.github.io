...
- Defined and delivered the product search experience end-to-end: a hybrid semantic + keyword engine combining vector similarity (embeddings + reranking) with lexical retrieval, which became the core discovery mechanism, driving a 27% improvement in add-to-cart conversion; serves ~100K multi-retailer (multi-tenant) requests/day at ~1s p95
- Shipped a customer-facing autonomous agent embedded in the e-commerce product that plans and executes multi-step tasks through multi-turn tool use for meal planning, cart building, and dietary profile tuning, acting directly on the live catalog and the shopper's cart
...
- Defined integration patterns and standards for connecting agents to external healthcare APIs, and built MCP servers for the tool layer and the multi-agent host acting as MCP client
...
- **[mlx-vlm](https://github.com/Blaizzy/mlx-vlm).** MLX Vision-Language Models. Open contribution, el primer port de la arquitectura `nemotron_parse` (C-RADIO + mBART).
...
- **[OpenInference](https://github.com/Arize-ai/openinference).** OpenTelemetry instrumentation for LLM applications. Contributor to the Anthropic instrumentor's token accounting.
- **[coremltools](https://github.com/apple/coremltools).** Apple's Core ML conversion toolkit. Open contribution to the TensorFlow frontend.
- **[tokenizers](https://github.com/huggingface/tokenizers).** Hugging Face's Rust tokenizer library. Open contribution to the BPE model loader.
- **[Inspect AI](https://github.com/UKGovernmentBEIS/inspect_ai).** The UK AI Security Institute's model evaluation framework. Open contribution to tool call parsing.
- **[LiteLLM](https://github.com/BerriAI/litellm).** Gateway and router across LLM providers. Open contributions to Bedrock error handling and router pre-call checks.
...
**AI/ML & Agents:** PyTorch, TensorFlow (Certified Developer), Scikit-learn, HuggingFace, LangGraph, LlamaIndex, CrewAI, Haystack, n8n, MCP (servers & clients), RAG, Prompt Engineering

**LLM Serving & Optimization:** vLLM, TGI, Ollama, MLX, Apple Neural Engine (coremltools, ANEForge), quantization (GGUF, MLX), LoRA / QLoRA, Unsloth, batch & real-time inference
...
**Evaluation & Observability:** LLM-as-judge, statistical significance testing, task-accuracy harnesses, offline & online eval pipelines, Arize, LangSmith, OpenTelemetry, Guardrails
...
**Databases:** PostgreSQL, MySQL, MongoDB, Cassandra, Firebase, Redis, Snowflake, BigQuery, Databricks
