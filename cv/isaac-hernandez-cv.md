...
Datasets, fine-tuned adapters, and quantized models published on [Hugging Face](https://huggingface.co/axiom-of-choice).
...
- **Spanish reasoning adapters** — Trained LoRA and QLoRA adapters on that dataset so Qwen3-4B, Qwen3-1.7B and Gemma-3-4B produce their chain-of-thought in Spanish, and published each for MLX, PEFT, and GGUF. Evaluated on a held-out Spanish GSM8K split with paired McNemar tests: Qwen3-4B gains 7.4 accuracy points alongside perfect Spanish (p = 0.0031), Gemma-3-4B gains 27.9 points plus a thinking mode the base model lacks, and Qwen3-1.7B trades 5.7 points for it, stated up front on its model card. [Adapters](https://huggingface.co/axiom-of-choice)
...
- **MLX** — Two merged, one in each repo. In `mlx-lm`, the LoRA trainer's batch shuffling now honors `seed=0`, which was silently ignored despite being the CLI default. In MLX core, `hadamard_transform` failed to compile on the GPU for n = 12/20/28, where a guard present on two stages was missing on the third and left a divide-by-zero in the step count. [PR #1661](https://github.com/ml-explore/mlx-lm/pull/1661) · [PR #4054](https://github.com/ml-explore/mlx/pull/4054)
...
- **In review** — Open contributions across `coremltools` (decomposing Softplus to avoid fp16 overflow on ANE), `mlx` core (e4m3 float8 NaN decoding), `mlx-lm` (gradient clipping, early stopping, incremental weight materialization on save), the vLLM GGUF plugin (weight loaders reading the tensor-parallel rank from a kwarg the v2 loader never passes, so every rank kept rank 0's shard), `tokenizers` (a BPE loader panic, in Rust), `inspect_ai` (tool call parsing for empty schemas), `langchain-google` (Gemini `service_tier` and JSON-encoded tool arguments), ANEForge, smolagents, CrewAI, LiteLLM, MCP servers, and gpt-researcher.
